...
void GLUI_Node::add_child_to_control(GLUI_Node *parent, GLUI_Control *child)
{
	...
	if (parent_control->hidden OR(parent_control->collapsible AND NOT parent_control->is_open))
	{
		child->hidden = true;
	}
}
...
void GLUI_Main::keyboard(unsigned char key, int x, int y)
{
	...
	if (key == '\t' AND !mouse_button_down AND(!active_control || !active_control->wants_tabs()))
	{
...
bool GLUI_Main::should_redraw_now(GLUI_Control *ctl)
{
	...
	{
	case buffer_front:
		return true; /* always draw in front-buffer mode */
	case buffer_back:
	{
...
void GLUI_Main::draw_raised_box2(int x, int y, int w, int h, float r, float g, float b)
{
	w = w + x;
	h = h + y;
	...
	glColor3f(r + 0.5, g, b);
	// glColor3ubv( bkgd_color );
	//  This draws the specified color border.
	glBegin(GL_LINE_LOOP);
	glVertex2i(x + 1, y + 1);
	glVertex2i(w - 1, y + 1);
	glVertex2i(w - 1, h - 1);
...
void GLUI_Main::activate_control(GLUI_Control *control, int how)
{
	...
	if (GLUI_Master.active_control_glui AND this != (GLUI_Main *)GLUI_Master.active_control_glui)
	{
		GLUI_Master.active_control_glui->deactivate_current_control();
	}
...
GLUI_Control *GLUI_Main::find_prev_control(GLUI_Control *control)
{
	...
	if (control == NULL)
	{ /* here we find the last valid control */
	...
	{ /* here we find the actual previous control */
		next_control = main_panel;
	...
	if (TEST_AND(this->flags, GLUI_SUBWINDOW))
	{
		...
		while (curr_glui)
		{
					...
					curr_glui->parent_window == this->parent_window)
			{
				...
				else if (TEST_AND(curr_glui->flags, GLUI_SUBWINDOW_TOP) AND(TEST_AND(this->flags, GLUI_SUBWINDOW_LEFT) OR
